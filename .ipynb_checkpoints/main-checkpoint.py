...
说明：模型结构全部保留在 model.py 中，本文件只负责配置、数据、训练和评估流程。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data", type=str, default="sample", help="数据集名称，位于 ./datasets 下")
    parser.add_argument("--data_dir", type=str, default=None, help="自定义数据集目录")
...
class DataModule:
    """数据加载、预处理、邻接矩阵构建和 DataLoader 生成。"""

    def __init__(self, dataset_name: str = "sample", data_dir: str = None):
        self.dataset_name = dataset_name
        self.data_dir = data_dir or os.path.join(DATASETS_ROOT, dataset_name)
    ...
    def generate_sample_data(
        ...
        num_kg_triples: int = 3000,
    ):
        print("正在生成 sample 示例数据集...")
        self.user_num = num_users
        self.item_num = num_items
        self.entity_num = num_entities
        self.relation_num = num_relations

        interactions = set()
        while len(interactions) < num_interactions:
            interactions.add((random.randint(0, num_users - 1), random.randint(0, num_items - 1)))
        interactions = list(interactions)
        random.shuffle(interactions)

        split_idx = int(len(interactions) * 0.8)
        self.train_data = interactions[:split_idx]
        self.test_data = interactions[split_idx:]

        kg_triples = set()
        while len(kg_triples) < num_kg_triples:
        ...
        self.kg_triples = list(kg_triples)
        ...
        print(f"数据生成完成：用户={num_users}, 物品={num_items}, 实体={num_entities}")
    ...
    def load_real_data(self):
        ...
        print(f"正在加载真实数据集：{self.dataset_name}")
        ...
        if os.path.exists(trn_file) and os.path.exists(tst_file):
            ...
            if not os.path.exists(train_file) or not os.path.exists(test_file):
                raise FileNotFoundError("数据集目录需要包含 trnMat.pkl/tstMat.pkl 或 train.txt/test.txt")
            ...
            self.train_mat = sp.coo_matrix((np.ones(len(rows), dtype=np.float32), (rows, cols)), shape=(self.user_num, self.item_num))
        ...
        self._build_adjacency_matrix()
        print(f"数据加载完成：用户={self.user_num}, 物品={self.item_num}, 实体={self.entity_num}, 关系={self.relation_num}")
...
class TrainingModule:
    ...
    def __init__(self, model: nn.Module, data_module: DataModule, learning_rate: float, weight_decay: float, device: str):
        ...
        self.train_losses = []
        self.test_recalls = []
        self.best_metrics = {"epoch": 0, "recall@20": 0.0, "ndcg@20": 0.0}
    ...
    def evaluate(self, test_loader, epoch: int):
        ...
        if metrics.get("Recall@20", 0) > self.best_metrics["recall@20"]:
            self.best_metrics = {
                "epoch": epoch,
    ...
    def _print_epoch_summary(self, epoch: int):
        if self.model.how2_stats is not None:
            ...
            print(
                ...
                f"Curvature(μ={curvature.float().mean().item():.4f}, σ={curvature.float().std().item():.4f})"
            )
        ...
        if self.model.how3_weights_cache is not None:
            ...
            print(f"[How3] Epoch {epoch}: Weights(μ={weights.mean().item():.4f}, σ={weights.std().item():.4f})")
    ...
    def train(self, epochs: int, eval_interval: int, batch_size: int):
        ...
        for epoch in range(epochs):
            if epoch == 0:
                print("[How3] Epoch 0: 进入 learning 阶段")
            if epoch == self.warmup_epochs:
                print("=" * 80)
                print(f"【第二阶段开始】Epoch {epoch + 1} 解冻 How1 参数，启动动态曲率优化")
                print("=" * 80)
        ...
        print(
            f"\n训练完成！最佳结果：Epoch {self.best_metrics['epoch']}, "
            f"Recall@20={self.best_metrics['recall@20']:.4f}, "
            f"NDCG@20={self.best_metrics['ndcg@20']:.4f}"
        )
...
class MultiMetricsEvaluator:
    """Precision/Recall/F1/HitRatio/NDCG/MAP 多指标评估。"""
...
def evaluate_model(config: Config, trainer: TrainingModule, data_module: DataModule) -> Dict[str, float]:
    ...
    metrics = trainer.evaluate(test_loader, epoch=config.epochs)
    for k in [5, 10, 20]:
        print(
            f"@{k}: "
            f"Precision={metrics.get(f'Precision@{k}', 0):.4f}, "
