...
"""
================================================================================
HDKCL - 模型模块 (Model Module)
================================================================================
核心模型架构，包含How1/How2/How3机制

核心创新机制:
- How1: 层次化曲率加权流形 (Hierarchical Curvature Weighting)
- How2: 曲率驱动的异步扩散 (Curvature-based Asynchronous Diffusion)
- How3: 任务对齐权重机制 (Task Alignment Weight)

流形空间:
- 双曲空间 (Hyperbolic): 适合层次结构
- 球面空间 (Spherical): 适合循环结构
- 欧氏空间 (Euclidean): 适合平坦结构
================================================================================
"""
...
class BakryEmeryCurvatureCalculator(nn.Module):
    ...
    该实现把图结构先验拆成两部分：
    1. Ollivier-Ricci 风格的局部度差异近似；
    2. 节点邻域熵拓扑先验。

    训练时再叠加可学习 BE potential，并可由任务梯度进行一阶曲率演化。
    """
    ...
    def build_topology_priors(self,
        ...
        """基于 KG 边构造 Ollivier-Ricci 近似和节点熵拓扑先验。"""
    ...
    def compute_curvature(self,
        ...
        """计算带拓扑先验、BE potential 和任务梯度演化的曲率。"""
...
class How1HierarchicalCurvatureWeighting(nn.Module):
    ...
    根据实体的曲率特征动态分配三种流形(双曲、球面、欧氏)的权重
    - 高曲率区域 -> 更多双曲权重 (适合层次结构)
    - 低曲率区域 -> 更多欧氏权重 (适合平坦结构)
    - 中等曲率 -> 更多球面权重 (适合循环结构)
    """
    ...
    def forward(self, entity_embeds: torch.Tensor, 
        ...
        Returns:
            curvature: 曲率值
            entity_weights: 实体级流形权重 [num_entities, 3]
            relation_weights: 关系级流形权重 [num_relations, 3]
        """
        # 计算曲率
        ...
        # 计算实体级权重 lambda_v^i = softmax(g_i(B(kappa_v), R_v))
...
class CurvatureDepthMapper(nn.Module):
    ...
    对应 sigmoid 调制的 L(v)=L_min+(L_max-L_min)*sigmoid(alpha*B_norm+beta)。
    默认 alpha=-1.5, beta=0，高曲率节点获得更浅传播深度。
    """
...
class HiCAPM(nn.Module):
    ...
    def __init__(self, user_num: int, item_num: int, 
        ...
        print("✓ How1改进的损失函数已初始化")
        print("✓ 统一曲率计算器已初始化")
        print("✓ How1层次化曲率加权流形已初始化")
        print("✓ How2异步扩散已初始化")
        print("✓ How3任务对齐已初始化")
